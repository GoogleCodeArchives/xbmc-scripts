...
Ce script nécessite un fichier de configuration rtsp2http.ini devant 
...
_version = "0.0.7"
_date = "1163373631"#modif alexsolex environs lignes 67 à 69, 110 à 112 et 28
...
def RecupParamServer ( serv ):
  ...
  du fichier de configuration rtsp2http.ini
  ...
  # Récupération du fichier rtsp2http.ini
  if os.path.isfile("Q:\\userdata\\rtsp2http.ini"):
    mon_fichier = "Q:\\userdata\\rtsp2http.ini"
  else:
    homedir = os.getcwd()
    if homedir.endswith(";"): homedir=homedir[:-1]
    mon_fichier = homedir + "\\"+"rtsp2http.ini"
...
def RecupOptions ():
  ...
  options = {}
  options["host"] = settings.get("general", "host") 
  options["port"] = settings.getint("general", "port") 
  options["queueSize"] = settings.getint("general", "queueSize") 
...
class principal:
  ...
  def run(self):
    ...
    options = RecupOptions()
    self.ServeurHTTP = HTTPServer (host = options['host'], port = options['port'], \
      queueSize = options['queueSize'], plagePortsClient = options['plagePortsClient'], \
